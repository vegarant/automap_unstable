...
def build_graph(noise_scaling):

    # The lambda in the objective function for generating adv. noise
    pl_noise_penalty = tf.compat.v1.placeholder(tf.float32, shape=(), name='noise_penalty')
    # pl_noise_scaling = tf.compat.v1.placeholder(tf.float32, shape=(), name='noise_scaling')
    ...
    # perturbation
    tf_rr_real = tf.Variable(noise_scaling*tf.random.uniform(tf_im.shape), name='rr_real', trainable=True)
    tf_rr_imag = tf.Variable(noise_scaling*tf.random.uniform(tf_im.shape), name='rr_imag', trainable=True)
    ...
    tf_solution = tf.compat.v1.placeholder(tf.complex64, shape=[N,N,1], name='actual')

    tf_obj = tf.nn.l2_loss(tf.abs(result_image - tf_solution)) - pl_noise_penalty * tf.nn.l2_loss(tf.abs(tf_rr))
...
tf_rr_real, tf_rr_imag, tf_input, tf_recovery, tf_obj, tf_adjoint =  build_graph(initial_noise_scaling)
...
with tf.compat.v1.Session() as sess:
    ...
    np.save(join(dest_data, f'im_rID_{runner_id}_im_nbr_{im_nbr}_rec_noiseless'), noiseless);
    
    fname_out_rec  = join(dest_plots, f'im_rID_{runner_id}_im_nbr_{im_nbr}_rec_noiseless.png'); 
    fname_out_orig = join(dest_plots, f'im_rID_{runner_id}_im_nbr_{im_nbr}_orig_noiseless.png'); 
    ...
    while(i < (max_num_noise_iter+1) and length < max_norm):
        ...
        while( length > max_norm and pert_nbr < len(norms)-1 ): 
            np.save(join(dest_data, f'im_rID_{runner_id}_im_nbr_{im_nbr}_pert_nbr_{pert_nbr}_adjoint'),
                    im_adjoint)
            np.save(join(dest_data, f'rr_im_rID_{runner_id}_im_nbr_{im_nbr}_pert_nbr_{pert_nbr}'),
                    rr_save)
            np.save(join(dest_data, f'im_rID_{runner_id}_im_nbr_{im_nbr}_pert_nbr_{pert_nbr}_rec'),
                    im_rec)
            np.save(join(dest_data, f'im_rID_{runner_id}_im_nbr_{im_nbr}_pert_nbr_{pert_nbr}_orig_p_noise'),
                    im_orig_p_noise)

            pert_nbr += 1
            max_norm = norms[pert_nbr];

            fname_out_rec = join(dest_plots, f'im_rID_{runner_id}_im_nbr_{im_nbr}_pert_nbr_{pert_nbr}_rec.png')
            fname_out_adjoint = join(dest_plots, f'im_rID_{runner_id}_im_nbr_{im_nbr}_pert_nbr_{pert_nbr}_adjoint.png');
            fname_out_orig_p_noise = join(dest_plots, f'im_rID_{runner_id}_im_nbr_{im_nbr}_pert_nbr_{pert_nbr}_orig_p_noise.png');
